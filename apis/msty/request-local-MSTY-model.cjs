#!/usr/bin/env node
/**
 * Script: request-local-MSTY-model.cjs
 * -----------------------------------
 ...
 *
 * Usage: pnpm node ai-labs/apis/msty/request-local-MSTY-model.cjs
 *
...
const findMonorepoRoot = () => {
  let dir = __dirname;
  const isMonorepoRoot = (d) => {
    if (!require('fs').existsSync(path.join(d, 'package.json'))) return false;
    const children = require('fs').readdirSync(d);
...
// Use .ts extension explicitly for TypeScript source compatibility in monorepo
const yamlUtilsPath = path.join(MONOREPO_ROOT, 'tidyverse/observers/utils/yamlFrontmatter.ts');
// Register ts-node to allow importing TypeScript source files
const { extractFrontmatter, writeFrontmatterToFile } = require('ts-node').register() && require(yamlUtilsPath);
...
async function getLLMCompletion(prompt, fileContent, filePath) {
  const payload = {
    prompt,
